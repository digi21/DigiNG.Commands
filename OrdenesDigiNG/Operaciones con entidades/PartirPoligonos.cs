...
{
    [LocalizableCommand(typeof(OrdenesDigiNG.Recursos), "PartirPoligonosName")]
    [LocalizableCommandInMenuAttribute(typeof(OrdenesDigiNG.Recursos), "PartirPoligonosTitle", MenuItemGroup.EditGroup9Group15)]
    public class PartirPoligonos : Command
    {
        public PartirPoligonos()
        {
            this.Initialize += new EventHandler(PartirPoligonos_Initialize);
            this.SetFocus += new EventHandler(PartirPoligonos_SetFocus);
            this.DataUp += new EventHandler<Digi21.Math.Point3DEventArgs>(PartirPoligonos_DataUp);
            this.EntitySelected += new EventHandler<EntitySelectedEventArgs>(PartirPoligonos_EntitySelected);
        }
        ...
        void PartirPoligonos_DataUp(object sender, Digi21.Math.Point3DEventArgs e)
        {
            DigiNG.SelectEntity(e.Coordinates, entidad => entidad is ReadOnlyLine);
        }

        void PartirPoligonos_SetFocus(object sender, EventArgs e)
        {
            SolicitaSeleccionaEntidad();
        }

        void PartirPoligonos_Initialize(object sender, EventArgs e)
        {
            SolicitaSeleccionaEntidad();
        }
